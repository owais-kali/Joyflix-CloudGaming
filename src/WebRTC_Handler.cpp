...
void WebRTC_Handler::OnClientConnect() {
    PC = std::make_unique<RTCPeerConnection>();
    ...
    PC->OnIceCandidate([](PeerConnectionObject* pco, const char* candidate, const char* sdpMid, const int sdpMLineIndex)
                       { WebRTC_Handler::GetInstance()->OnGotLocalIceCandidate(candidate, sdpMid, sdpMLineIndex); });
}
void WebRTC_Handler::OnClientDisConnect() {
    PC.reset();
}
          ...
          { WebRTC_Handler::GetInstance()->OnGotRemoteIceCandidate(candidate, sdpMLineIndex, sdpMid); }))
{
    ...
    signalling_handler.get()->RegisterOnConnectCallback(std::bind(&WebRTC_Handler::OnClientConnect,this));
    signalling_handler.get()->RegisterOnDisconnectCallback(std::bind(&WebRTC_Handler::OnClientDisConnect,this));
}
